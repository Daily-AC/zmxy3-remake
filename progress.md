...
状态标记：[ ] 待做 / [~] 在途 / [x] 已验收
1. [x] 爬塔背景生图换云纹雕柱塔身（平台梁已换，背景待生图接线）——主会话
2. [x] 乌鸦未被打就死：攻击范围/前端检测不匹配疑点 + 碰撞逻辑审计——codex-combat
3. [x] 跳跃抛体太快，适当放缓——codex-combat
4. [x] 掉落物去文字标签，只留物本体——主会话
5. [x] 小怪血条不在怪正上方（偏移）——主会话
6. [x] 队友名牌看不清 → 王者荣耀式（等级圈+血条+名字，参照图37）——主会话
7. [x] 伤害飘字打击感：打怪伤害橙色、自己掉血紫色、回血绿色，上飘消失（参照图38）+ N连击横幅（参照图39）——主会话
8. [x] 怪物无限眩晕不反击（用户第4次提出！）：打几次怪要反击——codex-combat（combo.ts 07-09 已修输入侧，怪物侧 hurt 锁仍可无限刷新，待修）
9. [x] 攻击特效缺失（参照图40，挥击红色刀光）——主会话
10. [x] 炼丹炉弹层点输入框穿透进九重天：弹层无事件吞噬层——主会话
11. [x] 老君回复横向溢出组件（中文无空格 Phaser 默认不折行，需 useAdvancedWrap）——codex-chat
