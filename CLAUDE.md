...
## 总纲（2026-07-08 晚用户二次拍板，最高优先级，覆盖一切散拍板）

0. **产品定位（19:0x 转向）**：游戏更名**「再续西游」**（repo 名不动）。定位从"复刻"转为"神似重制"——**机制、数值、手感保原版**（这三样继续对 AS3 真源），**观感统一重绘**：全 UI 毛笔书法字体（用户点名，方正字体=AI 味不切景）、缺口素材一律生图解决（不再投入抓取/提取原生资源包）。首页背景生图重画，**角色形象必须与原版一致，不天马行空**。
1. **素材边界（主会话技术拍板，2026-07-08 报备）**：角色/怪物动作帧 spritesheet 继续用已提取原版件（生图无法保证逐帧一致性，动画会闪变形）；背景/UI/图标/立绘/标题字全部生图统一水墨毛笔风，生图时喂原版件当风格与角色参照。已提取素材仍可用（它们就是拿到手的原版），但不再花工时挖新素材。
2. **范围**：关卡只做 L1+L2；L3/L4 摘除入口、代码留库。功能模块仍守"每模块一类实例"。
3. **新功能线（用户 19:0x 立项，23:3x 拍板后靠）**：好友系统 → 前置登录 → 开房 → 多人同房打关卡（联机 co-op）。涉及物品所有权、经验/灵魂分配算法、物品好友间转赠（服务端事务防双花）。**性能目标：10 人同房不卡**（服务器房间路由+扇出不仲裁、房主权威模拟、状态分层同步、压测判据写死）。**当前状态：后端大厅层已上线 home（/social/*，跨公网 P95=21ms），游戏侧登录/大厅 UI 冻结待启**——用户拍板"联机往后靠，先做最重要的东西"（单机核心体验优先）。
3b. **随时可看**：前端持续部署 home 子域名 **zaixu.qmledmq.cn:8443**（静态站 + home WSL ~/deploy-zm-frontend.sh 一键更新：pull→build→拷贝，WS 地址经 VITE_NPC_SERVER_URL 烘焙），用户随时随地手机/浏览器看最新构建。域名规划（2026-07-09 用户拍板）：前端 zaixu，正式后端预留 zaixu-api；zm 弃用；zm-dev 维持现役（agent-server+/social/*）直至正式化迁移。
3a. **素材卫生（20:2x 用户拍板）**：①现存坏素材一律弃用（截图裁切当组件、滤镜灰图一类），全库审计清单化处置；②**生图风格自洽铁律**——所有生图素材必须同一风格锚（style bible 落 docs/design/art-style.md，先定锚图再批量生成），禁止 a 风格 b 风格混出。
3c. **视觉基调（2026-07-09 用户拍板，黑客松评分"酷"占比大）**：本项目 web/UI 走**大胆炫技路线**，不走用户平日偏好的克制极简（用户明示那套记忆偏好本项目作废，别惯性做素版）——动效、光效、视差、仪式感转场往足了做，宁过勿素；燃系 keyart、毛笔书法、水墨×火焰的高对比氛围是基调。视觉终稿参照（用户已拍板）：`tmp/keyart/keyart-v1-dawn.png`（首页）+ `tmp/keyart/login-mock-v2.png`（登录文牒，四人探头版），入库件在 `game/public/assets/keyart/`。
3d. **屏幕流（2026-07-09 15:5x 用户拍板，赛内口径）**：**启动 → 登录（通关文牒）→ 选人 → 直进世界地图**。旧主菜单的罗列项（新的开始/继续游戏/游戏帮助/关于我们/退出游戏/联机共斗）**全部取消**；联机入口收进流内（世界地图上放开房/加入）。**存档系统赛内停用**（自动落固定槽即可），赛后恢复。前端打磨用户将开专门新会话执行。
4. 手感细调用户亲自来。
5. **好事慢磨**：可以做得慢，但不许糊弄。每项交付要么闭环要么给证据说明闭不了；"看起来差不多"不是完成。派发任务书必须写死判据，主会话终审必须亲看证据。
6. 旧"老玩家无感"条款由 0/1 条取代；docs/design/imperceptibility-gaps.md 账本中纯素材提取类欠账作废，机制/手感/表现类欠账仍有效。
