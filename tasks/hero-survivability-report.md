...
系数从判据带反推。参考 boss（二郎神 1299 魔法核击）在到关等级 + 中等炼装下掉 **39.7% 有效血**，普攻掉 **6.8%**，正落判据带。判据带写死进了测试，改曲线跑偏会红。exp 经济经续单二反编译后落进 `monsterExp.ts`（真值，multiplier 默认 1 不擅自放大）；自然通关轨迹到不了到关等级，偏差与最小调整方案（≈6×）见续单二，待拍板。
...
**最小调整方案（待拍板，未擅自放大）**：`monsterExp.ts` 留了 `CAMPAIGN_EXP_MULTIPLIER`，**默认 1（真值原样，不放大）**。因曲线高段陡峭（lv19→20 要 5000、lv20→21 要 10000），没有单一倍率能同时精确命中两关，但 **≈6×** 落 L3≈lv16 / L4≈lv20，双双进 lv15±1 / lv21±2 容差带。备选：给 boss 单独加里程碑 exp（更外科），或给关卡加波次/怪量（关卡设计，非 exp）。**建议 team-lead 拍板 multiplier=6**（一行改常量），或选备选方案。在拍板前保持 1，游戏内实际等级轨迹仍是 lv3/8/11。
