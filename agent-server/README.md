...
3. 用 `@anthropic-ai/claude-agent-sdk` 的 `tool()` + `createSdkMcpServer()`
   定义了三个自定义工具：`say`（必须每轮调用恰好一次）、`give_item`
   （按剧情调用）、`set_goal`（按剧情调用）。`query()` 的 `tools: []` 关掉了
   所有内置工具（Bash/Read/…），`allowedTools` 只放行这三个 MCP 工具，
   逼着模型只能通过工具调用输出结构化结果，不走自由文本 + 正则解析。
4. 工具的 handler 一被调用就立刻通过回调把 `npc_say`/`give_item`/
   `set_goal` 推给游戏端——不等模型这一轮彻底结束。
