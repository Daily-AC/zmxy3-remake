...
| `dialogue_textpanel_crop.png` | **2026-07-08 起：不再是裁图，是程序化生成的水墨条**（生成脚本见 `tasks/backpack-toast-report.md`）。原先这里放的是同一张过场截图裁出的底部文字条，本身烘焙着剧情台词和左右两侧人物碎片——这条本来就只该当"墨迹形状参照"用（见下一行的原始建议），却被直接当活体贴图加载进 Toast/DialogueBox，导致每次弹窗都带着烘焙文字和人物残影一起冒出来（用户反馈"杂乱悟空"）。全局搜过 vendor 库找独立的干净水墨符号：`export.SayInfo`（灰底小提示气泡，风格不符）、`GameCartoon`（手绘过场分镜，无法分离模板）均不合适，遂按下一行的原始建议**真正执行**——程序化生成一条无烘焙内容的水墨刷痕横条+细金线，风格致敬本行原图但不含任何照片内容。 | 保留原建议供追溯：不规则墨迹刷痕黑底、白字，没有矩形边框——面板背景应仿这个墨迹形状，不要做成直角描边框。 |
