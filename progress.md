...
- 15:5x **用户三拍板（总纲 3d 新增）**：①赛内砍存档系统（自动固定槽，SlotSelect 流程绕过、代码留库）；②屏幕流改**登录→选人→直进世界地图**，旧主菜单罗列项全部取消；③联机入口收进世界地图。已 SendMessage 转向 coop-shell（其 MainMenuScene 菜单项改动废弃回退）。两张 mock 用户拍板定稿（keyart-v1-dawn + login-mock-v2 四人探头版）。**用户预告：后端完备后将开专门新会话打磨前端**——本会话收口时需备好前端交接材料（mock 终稿+复刻要点+屏幕流 spec 均已在 CLAUDE.md 3c/3d 与 keyart 目录）。
- 15:5x C 棒关账（48406fd+97c7672+主会话补 688a860 单行页码）。观察：B/coop-shell/coop-sync/furnace-recipe 四棒的 report 均已落树、代码在工作树待各包装层验证提交——下一节点为四棒集中收口+主会话终审。
- 16:0x **B 棒代码验收通过（516300b）**：六件全落（bg11 对位/爬塔相机+32px 地面/乌鸦 0.75 比例+清洗版贴图根治灰蒙版/掉落 44px 裸图标/去拼音 toast/战斗内技能开技能树带 pause-resume），接缝拆分严谨（18 hunk 里 3 个 coop 接缝留树未收），我亲跑 575 绿复核。视觉终审（底/中/顶对位截图）待 coop-shell 流程手术完成后补——**当前 shell 处于半拆态（菜单已摘登录未接），全链临时不可玩，用户已知会准信**。coop-sync/furnace-recipe 两报告已落树（sync 复用 state/event 双消息+挂点清单在 §5；furnace 39 配方推导+FurnaceRecipeView+老君 craft_recipe 协议），待各包装层提交后终审。
...
## 2026-07-09 23:5x 用户亲玩反馈硬清单（说过多次的欠账在此归零，不销不关场）

状态标记：[ ] 待做 / [~] 在途 / [x] 已验收
1. [~] 爬塔背景生图换云纹雕柱塔身（平台梁已换，背景待生图接线）——主会话
2. [~] 乌鸦未被打就死：攻击范围/前端检测不匹配疑点 + 碰撞逻辑审计——codex-combat
3. [~] 跳跃抛体太快，适当放缓——codex-combat
4. [~] 掉落物去文字标签，只留物本体——主会话
5. [~] 小怪血条不在怪正上方（偏移）——主会话
6. [~] 队友名牌看不清 → 王者荣耀式（等级圈+血条+名字，参照图37）——主会话
7. [~] 伤害飘字打击感：打怪伤害橙色、自己掉血紫色、回血绿色，上飘消失（参照图38）+ N连击横幅（参照图39）——主会话
8. [~] 怪物无限眩晕不反击（用户第4次提出！）：打几次怪要反击——codex-combat（combo.ts 07-09 已修输入侧，怪物侧 hurt 锁仍可无限刷新，待修）
9. [~] 攻击特效缺失（参照图40，挥击红色刀光）——主会话
10. [~] 炼丹炉弹层点输入框穿透进九重天：弹层无事件吞噬层——主会话
11. [~] 老君回复横向溢出组件（中文无空格 Phaser 默认不折行，需 useAdvancedWrap）——codex-chat
12. [~] 老君聊天改 ChatGPT 式布局：用户右侧气泡、老君左侧平铺、可滚动看历史、一账号一 session 持久化——codex-chat
