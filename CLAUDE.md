...
3b. **随时可看 / 公网验收（2026-07-16 更新）**：测试构建持续部署到 home 独立子域名 **zaixu-dev.qmledmq.cn:8443**，所有浏览器验收必须针对该公网地址执行；localhost 只允许开发诊断，不能作为完成证据。正式站 **zaixu.qmledmq.cn:8443** 只在明确发布时更新，禁止测试构建覆盖。测试后端为 **zm-dev.qmledmq.cn:8443**（agent-server + `/social/*`）；正式后端预留 zaixu-api。
