...
3. **新功能线（用户 19:0x 立项）**：好友系统 → 前置登录 → 开房 → 多人同房打关卡（联机 co-op）。涉及物品所有权、经验/灵魂分配算法、物品好友间转赠（服务端事务防双花）。后端逻辑先行并行开发。**性能目标（20:2x 用户拍板）：10 人同房不卡**——架构按此设计（服务器房间路由+扇出不仲裁、房主权威模拟、状态分层同步 10-20Hz+插值、压测判据写死）。
